...
import { defineGetter, defineMemoGetter } from './helpers.js';

export default function makeDecorator(opts = {}) {
  const { querystring = nodeQuerystring, trustProxy = true } = opts;
  ...
  function proxyProtocol() {
    return this.headersDistinct['x-forwarded-proto']?.[0].trim() ?? protocol.call(this);
  }
