...
function makeDecorator() {
  ...
  function send(body) {
    if (Buffer.isBuffer(body)) {
      process.emitWarning(
        new Error('Sending a Buffer as a response body is deprecated.')
      );
    ...
    if (typeof body === 'string') {
      process.emitWarning(
        new Error('Sending a String as a response body is deprecated.')
      );
      if (!this.hasHeader('Content-Type')) {
    ...
    process.emitWarning(
      new Error(`Unsupported response body type ${typeof body}`)
    );
